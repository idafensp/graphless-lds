package oeg.upm.isantana.ldviz.graph.sql;
...
public class TriplesGraphSQL extends SPARQLSource {
...
//	private ArrayList<String> propQ;
	...
	public TriplesGraphSQL(String gn, String sparqlEndpointUrl, String namedGraph, NotRelevantProperties notrp, ResourcePropertiesSQL r, 
			CypherGraphHandler cypgh, RDFTypes rdftypes, RDFSLabelsSQL rdfslab, String url, String user, String pass) {
...
//		this.propQ = new ArrayList<String>();
	...
	public TriplesGraphSQL(String gn, String sparqlEndpointUrl, String namedGraph, NotRelevantProperties notrp, CypherGraphHandler cypgh, RDFTypes rdftypes, int lim, int off, int w) {
...
//		this.propQ = new ArrayList<String>();
		...
		for(String suburi : this.allSubjects)
		{
			...
			while (loop) {
				...
				while (results.hasNext()) {
					...
					double outw = this.getNomalizedOutgoingWeight(suburi, prop.getURI());
					if (outw == -1) {
					...
					if (!obj.isLiteral()) {
						double ingw = this.getNomalizedIngoingWeight(obj.asResource().getURI(), prop.getURI());
						if (ingw == -1) {
	...
	private double getNomalizedWeight(String tn, String res, String prop) {
		
		java.sql.ResultSet rset = sqlh.selectRPNormalized(tn, res, prop);
		...
		try {
			...
			while(rset.next())
			{
				norm = rset.getDouble("normalized");
				check++;
			}
			
			if(norm<0)
			{
				System.err.println("Weight not found at " + tn + "for " + res + "-" + prop + " returning 0");
			}
			
			if(check>1)
			{
				System.err.println("Multiple weight found at " + tn + "for " + res + "-" + prop);
			}
			return norm;
	...
	private double getNomalizedOutgoingWeight(String res, String prop) {
		return this.getNomalizedWeight(SQLConstants.NORM_RP_COUNT_OUT, res, prop);
	}
	private double getNomalizedIngoingWeight(String res, String prop) {
		return this.getNomalizedWeight(SQLConstants.NORM_RP_COUNT_ING, res, prop);
	}
